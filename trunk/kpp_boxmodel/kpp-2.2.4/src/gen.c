...
int dim;
  ...
  if ( (useLang != C_LANG)&&(useLang != MATLAB_LANG)&&(useLang != CUDA_C_LANG) ) return;
...
int dim;
  ...
  switch( useLang ) {
    ...
    case MATLAB_LANG: bprintf( InlineCode[ MATLAB_DATA ].code ); 
                 break;
    case CUDA_C_LANG:   bprintf( InlineCode[ C_DATA ].code ); 
                 break; 
  }
  FlushBuf();
...
int F_VAR, FSPLIT_VAR;
  ...
  if(useLang == CUDA_C_LANG){
      ...
      for(j = 0; j < EqnNr; ++j){
          if( useAggregate ){
              for(i = 0; i < SpcNr; ++i){
                  ...
                  if( Stoich[i][j] != 0 )
                      spc_idx[j * SpcNr + i] = (int)Stoich_Left[i][j];
                  else
                      spc_idx[j * SpcNr + i] = 0;
                  C_Inline("\t%d,", spc_idx[j * SpcNr + i]);
              }
          }
          else {
              for(i = 0; i < SpcNr; ++i){ 
      ...
      C_Inline("/* Computation of equation rates, vector A */");
      C_Inline("%s void d_Fun_A(double *V, double *F, double *Rct, double *A, int EqnNr, int VarNr, int SpcNr)",
               CUDA_C_function_qualifiers[1]);
      C_Inline("{");
      /* Kernel function body */
      C_Inline("\tint idx = blockDim.x * blockIdx.x + threadIdx.x;");
      NewLines(1);
      /* One CUDA thread per equation */
      C_Inline("/* One CUDA thread per equation */");
      C_Inline("\tif(idx < EqnNr){");
      C_Inline("\t\tdouble prod = Rct[idx];");
      C_Inline("\t\tfor(int i = 0; i < VarNr; ++i){");
      C_Inline("\t\t\tfor(int j = 1; j <= Spc_Idx[idx * SpcNr + i]; ++j)");
      C_Inline("\t\t\t\tprod *= V[i];");
      C_Inline("\t\t}");
      C_Inline("\t\tfor(; i < SpcNr; ++i){");
      C_Inline("\t\t\tfor(int j = 1; j <= Spc_Idx[idx * SpcNr + i]; ++j)");
      C_Inline("\t\t\t\tprod *= F[i - VarNr];");
      C_Inline("\t\t}");
      C_Inline("\t\tA[idx] = prod;");
      C_Inline("\t}");
      C_Inline("}");
      NewLines(2);
      /* CUDA Kernel for Fun_Vdot */
      /* Calculate vector V_dot */
      C_Inline("/* CUDA Kernel for Fun_Vdot */");
      C_Inline("/* Calculate vector V_dot */");
      C_Inline("%s void d_Fun_Vdot(double *Vdot, double *A, int EqnNr, int VarNr)",
               CUDA_C_function_qualifiers[1]);
      C_Inline("{");
      /* Kernel function body */
      C_Inline("\tint idx = blockDim.x * blockIdx.x + threadIdx.x;");
      NewLines(1);
      /* One thread per variable species */
      C_Inline("/* One thread per variable species */");
      C_Inline("\tif(idx < VarNr){");
      C_Inline("\t\tdouble sum = 0.0;");
      C_Inline("\t\tfor(int i = 0; i < EqnNr; ++i){");
      C_Inline("\t\t\tfor(int j = d_CCOL_STOICM[i]; j < d_CCOL_STOICM[i + 1]; ++j){");
      C_Inline("\t\t\t\tsum += A[i] * d_STOICM[j];");
      C_Inline("\t\t\t}");
      C_Inline("\t\t}");
      C_Inline("\t\tVdot[idx] = sum;");
      C_Inline("\t}");
      C_Inline("}");
      NewLines(2);
  }
  ...
  if(useLang == CUDA_C_LANG){
      ...
      C_Inline("\tstatic int gpuDevice = -1;     // Number of CUDA device");
      NewLines(1);
      /* Select CUDA device */
      C_Inline("\t/* Initialize CUDA related data */");
      C_Inline("\tif(gpuDevice == -1){");
      C_Inline("\t\tcudaDeviceProp gpuProps;");
      NewLines(1);
      C_Inline("\t\t/* Get information for available CUDA devices and select the best one */");
      C_Inline("\t\tgetBestGPUAvailable(&gpuDevice, &gpuProps);");
      C_Inline("\t}");
      C_Inline("\t/* Enable device */");
      C_Inline("\ttoolTestExec(cudaSetDevice(gpuDevice));");
      C_Inline("\t/* Initialize GPU timing events */");
      C_Inline("\tInitEvents();");
      NewLines(1);
      /* Allocate device memory & copy data */
      C_Inline("\t/* Allocate device memory & copy data */");
      C_Inline("\tsize = NVAR * sizeof(float);");
      C_Inline("\tgpuMalloc((void **)&d_V, size);");
      C_Inline("\tgpuMalloc((void **)&d_Vdot, size);");
      C_Inline("\tcopyToGPU(size, V, d_V);");
      C_Inline("\tsize = NFIX * sizeof(float);");
      C_Inline("\tgpuMalloc((void **)&d_F, size);");
      C_Inline("\tcopyToGPU(size, F, d_F);");
      ...
      C_Inline("\tint blocks_A = (NREACT - 1) / threads + 1;  // Blocks required");
      C_Inline("\tint blocks_Vdot = (NVAR - 1) / threads + 1;  // Blocks required");
      NewLines(1);
      C_Inline("\td_Fun_A<<<blocks_A, threads>>>(d_V, d_F, d_Rct, d_A, NREACT, NVAR, NSPEC);");
      C_Inline("\td_Fun_Vdot<<<blocks_Vdot, threads>>>(d_Vdot, d_A, NREACT, NVAR);");
      C_Inline("\t");
      NewLines(1);
      /* Get results from device */
      C_Inline("\t/* Get results from GPU */");
      C_Inline("\tsize = NVAR * sizeof(float);");
      C_Inline("\tcopyFromGPU(size, Vdot, d_Vdot);");
      NewLines(1);
      /* Free memory */
      C_Inline("\t/* Free GPU allocated memory */");
      C_Inline("\tgpuMemFree(d_V);");
      C_Inline("\tgpuMemFree(d_F);");
      C_Inline("\tgpuMemFree(d_Rct);");
      C_Inline("\tgpuMemFree(d_A);");
      C_Inline("\tgpuMemFree(d_Vdot);");
...
void GenerateRateLaws()
{
  ...
  switch( useLang ) {
    ...
    case MATLAB_LANG: bprintf( InlineCode[ MATLAB_RATES ].code ); 
                 break;
    case CUDA_C_LANG:  bprintf( InlineCode[ C_RATES ].code ); 
                 break;
  }
  FlushBuf();
...
int UPDATE_RCONST;
  ...
  switch( useLang ) {
    ...
    case MATLAB_LANG: bprintf( InlineCode[ MATLAB_RCONST ].code ); 
                 break;
    case CUDA_C_LANG:  bprintf( InlineCode[ C_RCONST ].code ); 
                 break;
  }
  FlushBuf();
...
void GenerateFortranInteface()
{
    ...
    //
    FreeVariable( INTEGRATE );
    /* change language back to CUDA_C_LANG */
    useLang = CUDA_C_LANG;
}
...
/*  if (useLang == MATLAB_LANG) return;  */
  ...
  switch( useLang ) {
    ...
    case MATLAB_LANG:bprintf( InlineCode[ MATLAB_UTIL ].code ); 
                 break;
    case CUDA_C_LANG:  bprintf( InlineCode[ C_UTIL ].code ); 
                 break;
  }
  FlushBuf();
 ...
 /* ExternDeclare( C_DEFAULT ); */
  ...
  switch( useLang ) {
    ...
    case MATLAB_LANG: bprintf( InlineCode[ MATLAB_GLOBAL ].code );
                 break;
    case CUDA_C_LANG:  bprintf( InlineCode[ C_GLOBAL ].code ); 
                 break;
  }
  FlushBuf();
...
int INITVAL;
  ...
  if ( (useLang==C_LANG)||(useLang==F77_LANG)||(useLang==F90_LANG)||(useLang==CUDA_C_LANG) )
      UseFile( initFile );
...
*/
  ...
  switch( useLang ) {
    ...
    case MATLAB_LANG: bprintf( InlineCode[ MATLAB_INIT ].code );
                 break;
    case CUDA_C_LANG: bprintf( InlineCode[ C_INIT ].code );
                 break;
  }
  FlushBuf();
...
void GenerateCUDAHeader()
{
    char buf[100];
    char headerfile[] = "";

    if ( useLang != CUDA_C_LANG ) return;

    sprintf( buf, "%s_CUDA_Utils.h", rootFileName );  
    CUDA_headerFile = fopen(buf, "w");
    if( CUDA_headerFile == 0 ) {
        FatalError(3,"%s: Can't create file", buf );
    } 

    UseFile( CUDA_headerFile );
  
    sprintf( headerfile, "%s/util/cuda_utils.h", Home);
...
int n;
  ...
  switch( useLang ) {
    ...
    case MATLAB_LANG: Use_MATLAB( rootFileName ); 
                 break; 
    /* 20131211 --ckannas Create CUDA C files. */
    case CUDA_C_LANG: Use_CUDA_C( rootFileName );
                 break;
...
/*  }*/
 ...
 GenerateBlas();
  ...
  /* Generate CUDA Utility header file */
  if(useLang == CUDA_C_LANG){
      GenerateCUDAHeader();
  }
