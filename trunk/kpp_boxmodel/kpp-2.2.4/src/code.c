...
/* Include Headers in .c  Files, except Makefile */
  ...
  /* Include Headers in .cu Files */
  if ( (bufname[blength-3]=='.')&&(bufname[blength-2]=='c'&&(bufname[blength-1]=='u') ) ){
    C_Inline("#include <stdio.h>");
    C_Inline("#include <stdlib.h>");
    C_Inline("#include <math.h>");
    C_Inline("#include <string.h>");
    C_Inline("#include \"%s_CUDA_Utils.h\"", rootFileName);
    C_Inline("#include \"%s_Parameters.h\"", rootFileName);
    C_Inline("#include \"%s_Global.h\"", rootFileName);
    if( useJacSparse )
       C_Inline("#include \"%s_Sparse.h\"", rootFileName);
...
FILE * fp;
  ...
  switch( useLang ) { 
    ...
    case CUDA_C_LANG: 
                 ...
                 sprintf( buf, "%s.cu", buf );
                 /* If CUDA file not available then use a C file */
                 fp = fopen( buf, "r" );
                 if(fp == 0)
                     sprintf( buf, "%s.c", oldbuf );
		 else
                    fclose(fp);
                 break;
