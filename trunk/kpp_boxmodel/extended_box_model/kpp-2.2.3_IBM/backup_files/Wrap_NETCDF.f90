MODULE Wrap_NETCDF
  ...
  USE netcdf 
  USE kpp_Parameters, ONLY: idim_spot, jdim_spot, kdim_spot, idim, jdim, kdim, ncells, &
...
SUBROUTINE Get_Boxes(vdata_kpp, vlocal_temperature, act_hour)
  IMPLICIT NONE
...
!!$  REAL, INTENT(out) :: local_temperature(idim, jdim, kdim)
  ...
  REAL, INTENT(out) :: vdata_kpp(ncells,ntot)
  REAL :: data_kpp(idim,jdim,kdim,ntot)
  REAL, INTENT(out) :: vlocal_temperature(ncells)
  REAL :: local_temperature(idim, jdim, kdim)
  ...
  INTEGER :: i, j, k, v, var_cnt

  CALL ReadDataNETCDF(data_var_netcdf, local_temperature, act_hour)

  ! MAP TEMPERATURE 3D -> 1D VECTOR
  v = 1
  DO k=kdim_loc_s,kdim_loc_e
     DO j=jdim_loc_s,jdim_loc_e
        DO i=idim_loc_s,idim_loc_e
           vlocal_temperature(v) = local_temperature(i,j,k)
           v = v + 1
        END DO
     END DO
  END DO
  ! END MAP TEMPERATURE 3D -> 1D VECTOR
  ...
  ! MAP DATA 3D -> 1D VECTOR
  v = 1
  DO k=kdim_loc_s,kdim_loc_e
     DO j=jdim_loc_s,jdim_loc_e
        DO i=idim_loc_s,idim_loc_e
           vdata_kpp(v,1:ntot) = data_kpp(i,j,k,1:ntot)
           v = v + 1
        END DO
     END DO
  END DO
  ! END MAP DATA 3D -> 1D VECTOR
