...
MODULE kpp_Integrator
  ...
  USE kpp_Parameters, ONLY: NVAR, NFIX, NSPEC, LU_NONZERO, &
       idim, jdim, kdim, idim_loc_s, jdim_loc_s, kdim_loc_s, &
       idim_loc_e, jdim_loc_e, kdim_loc_e, idim_spot, &
       jdim_spot, kdim_spot, NMONITOR, ncells
  USE kpp_Rates, ONLY: Update_RCONST
  USE Wrap_NETCDF, ONLY: lookup, n_from_file
  USE kpp_Global
  USE kpp_Monitor
  USE kpp_Util
  USE omp_lib
...
SUBROUTINE INTEGRATE( TIN, TOUT, & 
   ...
   INTEGER,       INTENT(IN) :: NHOUR ! Simulated hour 
   ...
   !REAL(kind=dp) :: time_initialize_init, time_initialize_final
   !REAL(kind=dp) :: time_savedata_init, time_savedata_final
   !REAL(kind=dp) :: time_integrate_init, time_integrate_final

   INTEGER       :: vv, tID, rowStart, rowEnd, N_over_THREADS

!!$   CALL SYSTEM_CLOCK(CLOCK_INTEGRATOR_INIT_1)
   !time_initialize_init = omp_get_wtime()

   IF (TIN == TSTART_INT) THEN
      IF (NHOUR == 0) THEN
...
!!$         !$OMP END PARALLEL
         ...
         DO vv=1,ncells
            ! set initial values with those of box(ii,jj,kk)
            VVARTOT(1:NVAR,vv) = vinitial_conc_from_file(vv,1:NVAR) * CFACTOR * 1000
         ENDDO
         !$OMP END DO
         !$OMP END PARALLEL
...
!!$         !$OMP END PARALLEL
         ...
         !$OMP PARALLEL DEFAULT(SHARED) &
         !$OMP PRIVATE(vv,var_cnt)
         !$OMP DO
         DO vv=1,ncells
            DO var_cnt=1,n_from_file
               ! set initial values with those of box(ii,jj,kk)
               VVARTOT(lookup(var_cnt),vv) = vinitial_conc_from_file(vv,lookup(var_cnt)) * CFACTOR * 1000
            ENDDO
         ENDDO
         !$OMP END DO
         !$OMP END PARALLEL
...
!!$      WRITE(6,991) (TIN-TSTART_INT)/(TEND_INT-TSTART_INT)*100, TIN, &
...
!!$   CALL SYSTEM_CLOCK(CLOCK_INTEGRATOR_INIT_2)
   !time_initialize_final = omp_get_wtime()

!!$   CALL SYSTEM_CLOCK(CLOCK_INTEGRATOR_SAVE_1)
   !time_savedata_init = omp_get_wtime()

   IF (NHOUR == 0 .AND. TIN == TSTART_INT) THEN
      ! Write species concentrations to file
      CALL SaveData()
   END IF

!!$   CALL SYSTEM_CLOCK(CLOCK_INTEGRATOR_SAVE_2)
   !time_savedata_final = omp_get_wtime()
...
!!$   ENDIF
   ...
   CLOCK_LU_FULL = 0
   CLOCK_LU_CALLS = 0
   CLOCK_LU_FULL_CALLS = 0
#endif

#ifdef FINE_TIMING
   CLOCK_PREPAREMATRIX_FULL = 0
   CLOCK_AXPY = 0
   CLOCK_KPPSOLVE = 0
   CLOCK_FUNTEMPLATE = 0
   CLOCK_JACTEMPLATE = 0
...
!!$   CALL SYSTEM_CLOCK(CLOCK_INTEGRATOR_CORE_1)
   !time_integrate_init = omp_get_wtime()
...
!!$         DO ii=idim_loc_s,idim_loc_e
   ...
   N_over_THREADS = INT(ncells/omp_get_max_threads())

#if defined(DETAILED_TIMING) && defined(FINE_TIMING)
   !$OMP PARALLEL DEFAULT(SHARED), PRIVATE(vv,ICNTRL,RCNTRL,ISTATUS,RSTATUS,tID,rowStart,rowEnd), LASTPRIVATE(RSTATUS_U), COPYIN(TIME,STEPMIN), &
   !$OMP& REDUCTION(+:CLOCK_LU, CLOCK_LU_FULL, CLOCK_LU_CALLS, CLOCK_LU_FULL_CALLS, CLOCK_PREPAREMATRIX_FULL, CLOCK_AXPY, CLOCK_KPPSOLVE, CLOCK_FUNTEMPLATE, CLOCK_JACTEMPLATE)
#elif defined(DETAILED_TIMING)
   !$OMP PARALLEL DEFAULT(SHARED), PRIVATE(vv,ICNTRL,RCNTRL,ISTATUS,RSTATUS,tID,rowStart,rowEnd), LASTPRIVATE(RSTATUS_U), COPYIN(TIME,STEPMIN), &
   !$OMP& REDUCTION(+:CLOCK_LU, CLOCK_LU_FULL, CLOCK_LU_CALLS, CLOCK_LU_FULL_CALLS)
#elif defined(FINE_TIMING)
   !$OMP PARALLEL DEFAULT(SHARED), PRIVATE(vv,ICNTRL,RCNTRL,ISTATUS,RSTATUS,tID,rowStart,rowEnd), LASTPRIVATE(RSTATUS_U), COPYIN(TIME,STEPMIN), &
   !$OMP& REDUCTION(+:CLOCK_PREPAREMATRIX_FULL, CLOCK_AXPY, CLOCK_KPPSOLVE, CLOCK_FUNTEMPLATE, CLOCK_JACTEMPLATE)
#else
!!$   !$OMP PARALLEL DO SCHEDULE(static), &
!!$   !$OMP& DEFAULT(SHARED), &
!!$   !$OMP& PRIVATE(vv,ICNTRL,RCNTRL,ISTATUS,RSTATUS), &
!!$   !$OMP& LASTPRIVATE(RSTATUS_U), &
!!$   !$OMP& COPYIN(TIME,STEPMIN), &
!!$   !$OMP& REDUCTION(+:ISTATS)

!!$   !$OMP PARALLEL DEFAULT(SHARED), PRIVATE(vv,ICNTRL,RCNTRL,ISTATUS,RSTATUS,tID,rowStart,rowEnd), LASTPRIVATE(RSTATUS_U), COPYIN(TIME,STEPMIN), REDUCTION(+:ISTATS) (pb at compilation)
   !$OMP PARALLEL DEFAULT(SHARED), PRIVATE(vv,ICNTRL,RCNTRL,ISTATUS,RSTATUS,tID,rowStart,rowEnd), COPYIN(TIME,STEPMIN), REDUCTION(+:ISTATS)
#endif

   tID = omp_get_thread_num()

   rowStart = ( tID * N_over_THREADS) + 1
   rowEnd   = ((tID+1) * N_over_THREADS)

!!$   WRITE(6,*) 'N_over_THREADS   = ', N_over_THREADS
!!$   WRITE(6,*) 'tID      = ', tID
!!$   WRITE(6,*) 'rowStart = ', rowStart
!!$   WRITE(6,*) 'rowEnd   = ', rowEnd

   DO vv=rowStart,rowEnd
!!$   DO vv=1,ncells
            ICNTRL(:)  = 0
            RCNTRL(:)  = 0.0_dp
            ISTATUS(:) = 0
...
!!$            TEMP = temperature_from_file(ii,jj,kk)
            TEMP = vtemperature_from_file(vv)
...
!!$                 RCNTRL,ICNTRL,RSTATUS,ISTATUS,IERR)
            ...
            CALL Rosenbrock(NVAR,VVARTOT(:,vv),TIN,TOUT,   &
                 ATOL,RTOL,                &
                 RCNTRL,ICNTRL,RSTATUS,ISTATUS,IERR)
...
!!$   CALL SYSTEM_CLOCK(CLOCK_INTEGRATOR_CORE_2)
   !time_integrate_final = omp_get_wtime()
...
TimeLoop: DO WHILE ( (Direction > 0).AND.((T-Tend)+Roundoff <= ZERO) &
   ...
   !CALL SYSTEM_CLOCK(CLOCK_TEST_FULL_1)
...
!~~~>   Compute the Jacobian at current time
   ...
   !CALL SYSTEM_CLOCK(CLOCK_TEST_FULL_2)
   !CLOCK_TEST_FULL = CLOCK_TEST_FULL + CLOCK_TEST_FULL_2 - CLOCK_TEST_FULL_1
...
!!$           CALL WAXPY(N,ros_A((istage-1)*(istage-2)/2+j), K(N*(j-1)+1),1,Ynew,1)
           CALL WAXPY_IBM(N,ros_A((istage-1)*(istage-2)*0.5+j), K(N*(j-1)+1),Ynew)
         END DO
         Tau = T + ros_Alpha(istage)*Direction*H ! Negligible
...
!!$         CALL WAXPY(N,HC,K(N*(j-1)+1),1,K(ioffset+1),1)
         CALL WAXPY_IBM(N,HC,K(N*(j-1)+1),K(ioffset+1))
       END DO
       IF ((.NOT. Autonomous).AND.(ros_Gamma(istage).NE.ZERO)) THEN
         HG = Direction*H*ros_Gamma(istage)
!!$         CALL WAXPY(N,HG,dFdT,1,K(ioffset+1),1)
         CALL WAXPY_IBM(N,HG,dFdT,K(ioffset+1))
       END IF
...
!!$         CALL WAXPY(N,ros_M(j),K(N*(j-1)+1),1,Ynew,1)
         CALL WAXPY_IBM(N,ros_M(j),K(N*(j-1)+1),Ynew)
   END DO
...
!!$        CALL WAXPY(N,ros_E(j),K(N*(j-1)+1),1,Yerr,1)
        CALL WAXPY_IBM(N,ros_E(j),K(N*(j-1)+1),Yerr)
   END DO
...
!!$   CALL WAXPY(N,(-ONE),Fcn0,1,dFdT,1)
   CALL WAXPY_IBM(N,(-ONE),Fcn0,dFdT)
...
#ifdef FINE_TIMING
   CALL SYSTEM_CLOCK(CLOCK_PREPAREMATRIX_FULL_1)
#endif
#ifdef DETAILED_TIMING
   CALL SYSTEM_CLOCK(CLOCK_LU_FULL_1)
   !CLOCK_LU_FULL_1 = omp_get_wtime()
#endif
...
#ifdef DETAILED_TIMING
   CALL SYSTEM_CLOCK(CLOCK_LU_FULL_2)
   !CLOCK_LU_FULL_2 = omp_get_wtime()
   CLOCK_LU_FULL = CLOCK_LU_FULL + CLOCK_LU_FULL_2 - CLOCK_LU_FULL_1
   CLOCK_LU_FULL_CALLS = CLOCK_LU_FULL_CALLS + 1
#endif
#ifdef FINE_TIMING
   CALL SYSTEM_CLOCK(CLOCK_PREPAREMATRIX_FULL_2)
   CLOCK_PREPAREMATRIX_FULL = CLOCK_PREPAREMATRIX_FULL + CLOCK_PREPAREMATRIX_FULL_2 - CLOCK_PREPAREMATRIX_FULL_1
#endif
...
#ifdef DETAILED_TIMING
   CALL SYSTEM_CLOCK(CLOCK_LU_1)
   !CLOCK_LU_1 = omp_get_wtime()
#endif
...
#ifdef DETAILED_TIMING
   CALL SYSTEM_CLOCK(CLOCK_LU_2)
   !CLOCK_LU_2 = omp_get_wtime()
   CLOCK_LU = CLOCK_LU + CLOCK_LU_2-CLOCK_LU_1
   CLOCK_LU_CALLS = CLOCK_LU_CALLS + 1
#endif
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 USE kpp_Parameters, ONLY: NVAR, LU_NONZERO
 USE kpp_Global, ONLY: FIX, RCONST, TIME
...
#ifdef FINE_TIMING
   CALL SYSTEM_CLOCK(CLOCK_FUNTEMPLATE_1)
#endif

   Told = TIME
   TIME = T
   !CALL Update_SUN()
   CALL Update_RCONST() ! NOTE: THIS SHOULD BE COMMENTED ?
   CALL Fun( Y, FIX, RCONST, Ydot )
   TIME = Told

#ifdef FINE_TIMING
   CALL SYSTEM_CLOCK(CLOCK_FUNTEMPLATE_2)
   CLOCK_FUNTEMPLATE = CLOCK_FUNTEMPLATE + CLOCK_FUNTEMPLATE_2 - CLOCK_FUNTEMPLATE_1
#endif
...
#ifdef FINE_TIMING
    CALL SYSTEM_CLOCK(CLOCK_JACTEMPLATE_1)
#endif
...
#ifdef FINE_TIMING
    CALL SYSTEM_CLOCK(CLOCK_JACTEMPLATE_2)
    CLOCK_JACTEMPLATE = CLOCK_JACTEMPLATE + CLOCK_JACTEMPLATE_2 - CLOCK_JACTEMPLATE_1
#endif
