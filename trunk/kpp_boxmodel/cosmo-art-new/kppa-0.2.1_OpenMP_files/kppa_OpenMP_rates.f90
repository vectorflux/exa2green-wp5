!--------------------- BEGIN kppa_OpenMP_rates.f90 BEGIN ---------------------
! @file kppa_OpenMP_rates.f90                                                 
! @author charlesj                                                            
! @date 2014-12-18 14:51:31.917113                                            
...
! BEGIN INLINE declared at /users/charlesj/KPP_BOXMODEL/cosmo-art-new/config/radm2_kpp_eleni_0714_kppa.def:316,1 
...
!C
 ...
 FUNCTION TROE2( K0300, Q, KU300, R, M, T )
!C CALCULATION OF RATE CONSTANTS FOR TROE REACTIONS
!      IMPLICIT REAL(8) (A-Z)
      REAL(8) :: K0300, Q, KU300, R, M, T, TROE
      ...
      K0M = K0 * M
      KK = K0M / KU
      LGKK = 0.434294481E0 * LOG(KK)
      E = 1.E0 / ( 1.E0 + LGKK*LGKK )
      F = 0.6E0 ** E
      TROE = F * K0M / ( 1.E0 + KK )
END FUNCTION TROE
!C
...
FUNCTION EQT( K0300, Q, KU300, R, M, T, A, B )
      !IMPLICIT REAL(8) (A-Z)
      REAL(8) :: K0300, Q, KU300, R, M, T, A, B, EQT
      REAL(8) :: kh
...
!C SPECIAL RATE CONSTANTS
      !IMPLICIT REAL(8) (A-Z)
      REAL(8) :: A0,B0,A2,B2,A3,B3,M,T, SPEZ
      REAL(8) :: k0, k2, k3
      REAL(8), PARAMETER :: M = 2.55e19
      K0 = A0*DEXP(B0/T)
      K2 = A2*DEXP(B2/T)
...
! END INLINE declared at /users/charlesj/KPP_BOXMODEL/cosmo-art-new/config/radm2_kpp_eleni_0714_kppa.def:316,1 
...
!-----------------------------------------------------------------------------
  SUBROUTINE Rates(time, idx, rct)
    ...
    rct(31) = TROE( 1.8E-31, 3.2e0, 4.7E-12, 1.4e0, M, TEMP )
    rct(32) = 2.2E-13 * exp(620./TEMP) + 1.9E-33 * M * exp(980./TEMP)
    rct(33) = 3.08e-34*exp(2820./TEMP)+2.66e-54*M*exp(3180./TEMP)
    rct(34) = 3.30E-12 * exp( -200. / TEMP )
    rct(35) = TROE( 7.0e-31, 2.6e0, 1.5e-11, 0.5e0, M, TEMP )
    rct(36) = 3.30E-39 * exp( 530. / TEMP )
    rct(37) = 1.40E-13 * exp( -2470. / TEMP )
    rct(38) = 1.80E-11 * exp( 110. / TEMP )
    rct(39) = 2.50E-14 * exp( -1230. / TEMP )
    rct(40) = 2.5e-12
    rct(41) = TROE( 2.2e-30, 4.3e0, 1.5e-12, 0.5e0, M, TEMP )
    rct(42) = EQT(2.2e-30,4.3e0,1.5e-12,0.5e0,M,TEMP,9.09e+26,11200.e0)
    rct(43) = HET(1)
    rct(44) = TROE( 2.6e-30, 3.2e0, 2.4e-11, 1.3e0, M, TEMP )
    rct(45) = SPEZ(7.2e-15,785.e0,4.1e-16,1440.e0,1.9e-33,725.e0,M,TEMP)
    rct(46) = 1.30E-12 * exp( 380. / TEMP )
    rct(47) = 4.80E-11 * exp( 250. / TEMP )
    rct(48) = TROE( 3.0e-31, 3.3e0, 1.5e-12, 0.0e0, M, TEMP )
    rct(49) = 1.5E-13 * ( 1.0 + 2.439E-20 * M )
    ...
    rct(156) = ARR2(1.19E-12,490.)
    rct(157) = ARR2(1.01E-15,-736.)
    rct(158) = 4e-12
    rct(159) = 1.5e-11
    rct(160) = ARR2(3.56E-14,708.)
    rct(161) = ARR2(7.40E-13,765.)
    rct(162) = 1.2e-12
    rct(163) = 1.7e-10
    rct(164) = 1.22e-11
    rct(165) = 2e-16
    rct(166) = 4e-12
    rct(167) = 1.5e-11
    rct(168) = ARR2(3.56E-14,708.)
    rct(169) = ARR2(7.40E-13,765.)
    rct(170) = 1.2e-12
    rct(171) = ARR2(2.43E-12,360.)
    rct(172) = ARR2(2.05E-13,1300.)
    rct(173) = 2e-12
    rct(174) = 1e-10
    rct(175) = 1.3e-11
    ...
    rct(177) = 0.5*(1.36E-15 * exp( -2112. / TEMP ) + 7.51E-16 * exp( -1521. &
        / TEMP ))
    rct(178) = ARR2(2.54E-12,360.)
    rct(179) = ARR2(1.82E-13,1300.)
    rct(180) = 2e-12
    rct(181) = TROE( 9.7e-29, -5.6e0, 9.3e-12, -1.5e0, M, TEMP )
    rct(182) = TROE( 9.7e-29, -5.6e0, 9.3e-12, -1.5e0, M, TEMP &
        )/(ARR2(9.0E-19,14000.))
    rct(183) = 3.6e-12
    rct(184) = 3e-11
    rct(185) = 3e-12
    rct(186) = ARR2(5.60E-12,270.)
    rct(187) = ARR2(1.9E-13,500.)
    rct(188) = ARR2(9.6E-12,-234.)
    rct(189) = &
...
END MODULE kppa_OpenMP_rates
!----------------------- END kppa_OpenMP_rates.f90 END -----------------------
