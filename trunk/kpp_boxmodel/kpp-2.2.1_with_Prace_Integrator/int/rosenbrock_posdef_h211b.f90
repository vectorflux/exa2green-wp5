...
!~~~> kk value for aggressive time stepping increase calculation (Soederling-2003,ACM)
   ...
   END IF
   !IF (l_H211b) THEN
   !   PRINT * , 'USAGE OF H211b TIME STEP FILTER', RCNTRL(9)
   !ELSE
   !   PRINT * , 'USAGE OF H110 TIME STEP FILTER', RCNTRL(9)
   !ENDIF
...
!~~~> Input: integration parameters
   ...
   KPP_REAL, INTENT(IN) :: kk, bb ! 20120302_mz_dt 
   LOGICAL, INTENT(IN)  :: l_H211b! 20120302_mz_dt
...
!~~~>   Compute the Jacobian at current time
   CALL JacTemplate(T,Y,Jac0 )  ! 20121011_mz_dt
...
!SUBROUTINE FunTemplate( T, Y, Ydot, Autonomous )  ! 20121011_mz_dt
!GSF+
...
!   END IF
! 20121011_mz_dt-
   if (i_do_lin_interp>0) then 
      do i=1, Nreact
         x = rconst_dt(i) - rconst_prev(i)
         rconst(i) = rconst_prev(i) + x * T/period
      enddo
   endif
!cy_gf_20121129-
...
!cy_gf_20121129-
    Told = TIME
    TIME = T
!      CALL Update_SUN() ! 20121011_mz_dt
!      CALL Update_RCONST() ! 20121011_mz_dt
#ifdef FULL_ALGEBRA    
