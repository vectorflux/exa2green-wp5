...
full processor's name plus its microarchitecture.
\end{weSay}
...
%%% *** Manuel ***
\begin{weSay}
We have accordingly replaced ``power dissipation'' by ``power consumption'' along the manuscript to avoid any confusion.
\end{weSay}
...
%%% *** Joseph ***
\begin{weSay}
We agree with the rewiever. Indeed we tried to shorten the COSMO-ART's explanation due to space restrictions in the last version of the paper. However
with the page limit extension we added more details about the nature of the application to better understand its behaviour.
\end{weSay}
...
%%% *** Will ***
\begin{weSay}
Indeed, there is a huge amount which could be said about COSMO, and COSMO-ART in particular, 
about strong and weak scalability, energy scalability, along with the underlying science.  
However, going into this detail would make this an entirely different paper, and one that has, 
at least for COSMO, already been published in several forums. %   Moreover, such a comprehensive 
%analysis would have taken far more than eight pages.

We had a modest goal in this paper of evaluating the COSMO-ART configuration, which is a benchmark 
for production at MeteoSwiss, in terms of energy-to-solution and time-to-solution.  To this end we 
have used state-of-the-art profiling and energy measurement equipment.   In subsequent papers, we 
plan to address a wider spectrum of energy issues.    
\end{weSay}
...
%%% *** Will/Joseph ***
\begin{weSay}
Indeed, there is only a tangential relationship between [11] and [20] and our paper, and we would be happy to remove these.  There is in fact very little related work on energy evaluations of real applications; most work in the field limits itself to artificial toy examples, which is definitely not our topic of interest.  Actually, we have added one reference that helps us to justify the use of the most appropriate wattmeters for an application like COSMO-ART.
%We are trying to find more references on infrastructure and tool support, and hope to add these in the final version
...
%%% *** Joseph ***
\begin{weSay}
We have described in detail the nodes' architectures that this platform is comprised of.
We only used a subset of 52 in our experiments, that is why the reviwer was confused. We now rephrased the explanation in order to be more precise.
\end{weSay}
...
%%% *** Joseph ***
\begin{weSay}
The reviwer is completely right. Thus, we have accordingly modified the text in order to avoid comparisons between SB and IB processors.
\end{weSay}
...
%%% *** Manuel/Sandra ***
\begin{weSay}
We have added the details about the Turbo Boost feature and power saving mechanisms for the used platforms.
\end{weSay}
...
%%% *** Will/Joseph ***
\begin{weSay}
The E3METER metering framework records averaged power data every 5 minutes, this is as much as we can do in MONCH and PILATUS platforms to collect power consumption data.
On the other hand, Figure 5 represents the power drawn of TINTORRUM, for this case a PDU with a sampling rate of 1 Hz was used instead.
\end{weSay}
...
%%% *** Manuel/Sandra ***
\begin{weSay}
We have added information about the accuracy of the APC8653 PDU.
\end{weSay}
...
%%% *** Manuel/Sandra ***
\begin{weSay}
We noticed the problem and fixed it accordingly along the manuscript. We did a misuse of the blocking/polling terms.
We have replaced them to ``aggressive'' and ``degraded'', respectively, since exactly reflect the experiments of our paper. 
For the second comment, the reviewer is completely right, however even with  the ``degraded'' policy 
the CPU utilization is still reduced to 50\,\% and thus allows us to save energy.
\end{weSay}
...
%%% *** Joseph ***
\begin{weSay}
We have rephrased the paragraph accordingly to avoid any confusion.
\end{weSay}
...
%%% *** Joseph ***
\begin{weSay}
We have fixed this issue along the manuscript. Now we employ the platform's name for comparing them.
\end{weSay}
...
%%% *** Will ***
\begin{weSay}
We avoided to make comparisons between MONCH and PILATUS platforms - so far we have no more information about the scalability of COSMO-ART. We modified the text accordingly.
...
%%% *** Manuel/Sandra ***
\begin{weSay}
The reviewer is right in this sense. The processors are not idle during blocking MPI routines, however a reduction of the CPU utilization is observed (50\,\%) even with the degraded policy that we used.
\end{weSay}
