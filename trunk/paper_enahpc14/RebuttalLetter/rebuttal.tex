...
%%% *** Manuel/Sandra ***
\begin{weSay}
We noticed the problem and fixed it accordingly along the manuscript. We misused blocking/polling terms.
Now we have replaced the ``polling'' and ``blocking''` OpenMPI policies to ``aggressive'' and ``degraded'', 
respectively. The reviewer is completely right on this issue, however even with 
the ``degraded'' policy the CPU utilization is reduced to 50\,\% and thus allows us saving energy.
\end{weSay}
