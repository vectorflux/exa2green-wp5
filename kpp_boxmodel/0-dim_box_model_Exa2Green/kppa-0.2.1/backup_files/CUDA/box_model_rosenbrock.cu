/*-------------------- BEGIN box_model_rosenbrock.cu BEGIN --------------------*/
/* @file box_model_rosenbrock.cu                                               */
/* @author charlesj                                                            */
/* @date 2014-11-28 20:16:23.489335                                            */
...
#include "box_model_blas.h"
#include "box_model_rates.h"
#include "box_model_function.h"
#include "box_model_decomp.h"
#include "box_model_solve.h"
#include "box_model_jacobian.h"
...
#define TAG (double)103.0 // 13 April 2010
...
void Integrate(size_t const ncells, double d_var[81], double d_fix[1], size_t
    ...
    abstol[81], double const  reltol[81], int idata[20], double rdata[20])
{
    ...
    double CHI,SD,CD,FP,OM,SH,SH1,DEKL,ZEIT,GB,GL;
	...
	DEKL = 23.45e0*sinf((280.1e0+0.981e0*TAG)*0.01745329e0);
	FP = 0.01745329e0;
	SD = sinf(DEKL*FP);
	CD = cosf(DEKL*FP);
	OM = (12.e0-(ZEIT/60.+GL/15.e0))*0.261799388e0;
	SH = sinf(GB*FP)*SD+cosf(GB*FP)*CD*cosf(OM);
	...
	CHI=3.141592/180.*CHI;
        ...
        Rates(ncells32, T, idx, d_rct, CHI);
        ...
        if (!autonomous) {
            ...
            Rates(ncells32, T+delta, idx, d_rct, CHI);
        ...
        do {
            ...
            for (int i=1; i<nStage; ++i) {
                ...
                if (F[i]) {
                    ...
                    if(!autonomous) {
                        Rates(ncells32, tau, idx, d_rct, CHI);
                    }
...
/*----------------------- END box_model_rosenbrock.h END ----------------------*/
