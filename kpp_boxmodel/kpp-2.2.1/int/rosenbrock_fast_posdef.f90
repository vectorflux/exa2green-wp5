...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
! Update by A. Sandu providing the Rang3 solver
! Modifications by D. Taraborrelli 2012: 
!   - time stepping with H211b controller (default = disactivated, 20-30% slower)
!   - replacement of WAXPY calls with compact FORTRAN95 operations with arrays
!     (not done yet!!!!!) 
...
! ~~~~ Local variables
   ...
   KPP_REAL :: Err, Yerr(N)
   KPP_REAL :: ErrOld, FacOld, Ymax, bb, Tol ! 20120302_mz_dt
...
! 20120302_mz_dt+
   REAL :: tinit, tfinish
   REAL :: t_stages, t_preparematrix, t_jactemplate
   REAL :: t_funtimeder, t_funtemplate
! 20120302_mz_dt-
...
!   call cpu_time(tinit) ! 20120302_mz_dt
...
!   call cpu_time(t_funtemplate) ! 20120302_mz_dt
...
!   call cpu_time(t_funtimeder) ! 20120302_mz_dt

!~~~>   Compute the Jacobian at current time
   CALL JacTemplate(T,Y,Jac0)
   ISTATUS(Njac) = ISTATUS(Njac) + 1

!   call cpu_time(t_jactemplate) ! 20120302_mz_dt
...
!   call cpu_time(t_preparematrix) ! 20120302_mz_dt
...
!   call cpu_time(t_stages) ! 20120302_mz_dt
...
!20120302_mz_dt+
  DO i=1,N
    ...
    IF (VectorTol) THEN
      Tol = AbsTol(i)+RelTol(i)*Ymax
    ELSE
      Tol = AbsTol(1)+RelTol(1)*Ymax
    END IF
  END DO
!to initiate
  FacOld = 1
...
!   Fac  = MIN(FacMax,MAX(FacMin,FacSafe/Err**(ONE/ros_ELO))) ! asymptotic step size controller

!20120302_mz_dt+
!~~~> H211b step size controller (b=2.) advised by Söderlind
  bb = 1.
   Fac  = (FacSafe/(Err))**(1./(bb*ros_ELO))* &
          (FacSafe/(ErrOld))**(1./(bb*ros_ELO))* &
          (FacOld)**(-1./bb)
  FacOld = Fac
  ErrOld = Err
!20120302_mz_dt-
...
!   call cpu_time(tfinish) ! 20120302_mz_dt

!print *,'HH=',H,'Fac=',Fac,'predicted Fac=', FacSafe/Err**(ONE/ros_ELO)
!print *,'Fac=',Fac, '--> HH=', H
!print *,'frac t_funtemplate  =', (t_funtemplate-tinit)/(tfinish-tinit)
!print *,'frac t_funtimeder   =', (t_funtimeder-t_funtemplate)/(tfinish-tinit)
!print *,'frac t_jactemplate   =', (t_jactemplate-t_funtimeder)/(tfinish-tinit)
!print *,'frac t_preparematrix =', (t_preparematrix-t_jactemplate)/(tfinish-tinit)
!print *,'frac t_stages        =', (t_stages-t_preparematrix)/(tfinish-tinit)
!print *,'tinit                =', tinit
