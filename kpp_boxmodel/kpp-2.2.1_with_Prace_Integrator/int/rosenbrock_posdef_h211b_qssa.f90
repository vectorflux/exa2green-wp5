...
!  USE KPP_ROOT_Parameters, ONLY: NVAR, NFIX, NSPEC, LU_NONZERO
  USE KPP_ROOT_Parameters, ONLY: NVAR, NFIX, NSPEC, LU_NONZERO, NREACT
!GSF-
  USE KPP_ROOT_Global
  IMPLICIT NONE
  PUBLIC
...
!    RCNTRL(8)  -> bb value for aggressive time stepping increase 
!                     calculation (Soederling-2003,ACM)
!    RCNTRL(9)  -> kk value for aggressive time stepping increase
!                  calculation (Soederling-2003,ACM)
!    RCNTRL(10) -> thres_tau, threshold value for QSSA species lifetime 
!    RCNTRL(11) -> RelTol (user-defined)
!    RCNTRL(12) -> AbsTol (user-defined)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!
!
!    OUTPUT ARGUMENTS:
...
!~~~>  Arguments
   ...
   KPP_REAL, INTENT(INOUT)    :: AbsTol(N),RelTol(N) ! MZ_CY
...
!~~~>  Local variables
   ...
   KPP_REAL :: kk, bb, thres_tau ! MZ_CY
   LOGICAL  :: l_H211b
...
!~~~> Usage of H110 or H211b time stepping filter
   ...
   IF (RCNTRL(8) == ZERO.AND.RCNTRL(9) == ZERO) THEN
      l_H211b = .FALSE.
   ENDIF
!~~~> bb value for aggressive time stepping increase calculation (Soederling-2003,ACM)
   ...
   ELSE
      ...
      RETURN
   END IF
!~~~> kk value for aggressive time stepping increase calculation (Soederling-2003,ACM)
...
!~~~>  Threshold for the QSSA species lifetimes 
   ...
   ELSEIF (RCNTRL(10) > ZERO) THEN
      thres_tau = RCNTRL(10)
   ELSE
      PRINT * , 'User-selected thres_tau: RCNTRL(10)=', RCNTRL(9)
      CALL ros_ErrorMsg(-4,Tstart,ZERO,IERR)
      RETURN
   END IF
!~~~>  Changing the default tolerances 
   IF (RCNTRL(11) > ZERO) THEN
      DO i=1,UplimTol
         RelTol(1:N) = RCNTRL(11) 
      ENDDO
   END IF
   IF (RCNTRL(12) > ZERO) THEN
      DO i=1,UplimTol
         AbsTol(1:N) = RCNTRL(12)
      ENDDO
!   ELSEIF (RCNTRL(11) > ZERO) THEN
!      DO i=1,UplimTol
!         AbsTol(1:N) = 1E2 * RelTol(1:N) ! similar to Eller et al. (2009) that used 1E4 instead
!      ENDDO      
   END IF
...
!  Integration parameters
        ...
        l_H211b, kk, bb, istep, thres_tau,       & ! MZ_CY
        l_doqssa, l_daeqssa, l_plainqssa, l_exqssa, l_symqssa, & ! MZ_CY
        posdef,                       & ! MZ_CY
!  Error indicator
        IERR)
...
!  USE Fun_Split    !SOS Maybe I should put that in  template (FunSplitTemplate)
...
!MZ_CY+
   KPP_REAL :: P(N), L(N), LC(N), Iqssa(N), Css(N), Y0(N), Y1(N), Y2(N), Y3(N) ! Production rate, Destruction frequency and Concentration at steady state 
...
!MZ_CY-
   ...
   KPP_REAL :: Err, Yerr(N)
   KPP_REAL :: ErrOld, FacOld, Ymax ! MZ_CY
...
! estimate of the induction period according to Turanyi et al.(1993)
      do i=1,N
         IF (Iqssa(i) < 1E-2 .AND. L(i) /= 0. .AND. tlife(i) < thres_tau) THEN
            tau_qssa(i) = tlife(i)
...
!~~~>   Compute the function at current time
      CALL FunTemplate(T,Y,Fcn0, Autonomous ) ! MZ_CY 
...
!~~~>  Compute the function derivative with respect to T
   IF (.NOT.Autonomous) THEN
      CALL ros_FunTimeDerivative ( T, Roundoff, Y, &
                Fcn0, dFdT , Autonomous )  ! MZ_CY
   END IF

!~~~>   Compute the Jacobian at current time
   CALL JacTemplate(T,Y,Jac0, Autonomous )  ! MZ_CY
...
!            K(N*(j-1)+1),1,Ynew,1)
         ...
         CALL FunTemplate(Tau,Ynew,Fcn, Autonomous )  ! MZ_CY
         ISTATUS(Nfun) = ISTATUS(Nfun) + 1
...
!~~~> Input arguments
   ...
   LOGICAL, INTENT(IN) :: Autonomous ! MZ_CY
!~~~> Output arguments
...
!~~~> Local variables
   ...
   CALL FunTemplate(T+Delta,Y,dFdT, Autonomous )  ! MZ_CY
   ISTATUS(Nfun) = ISTATUS(Nfun) + 1
...
!SUBROUTINE FunTemplate( T, Y, Ydot, Autonomous )  ! MZ_CY
!GSF+
